...
export default function LearningPath({
  ...
  const lastLessonId =
    lessonGroups && lessonGroups[lessonGroups.length - 1]?.id;
  if (!course) {
    return (
...
const RenderLesson = ({
  ...
  const { learnedLessons, inProgressLesson, clearInProgressLesson } =
    useLearningStore();
  const completedResult = learnedLessons?.find(
    ...
    >
      ...
      <Dialog open={showDialog} onOpenChange={() => setShowDialog(false)}>
        <DialogContent>
          ...
          <Text>
            You have an unfinished lesson in progress. If you continue with this
            lesson, your previous progress will be lost!
          </Text>
          <DialogFooter>
