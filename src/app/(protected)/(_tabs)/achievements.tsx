...
import { FlatList, Image, Pressable, RefreshControl, SafeAreaView, View } from 'react-native';
        ...
        {selectedItem && (
          <DialogContent className="mx-3 rounded-2xl">
            <View className="rounded-2xl items-center  gap-5">
              <Text className="text-2xl font-bold text-primary">{selectedItem.name}</Text>
              ...
              <Image
                ...
                style={{
                  height: 240,
                  aspectRatio: 86 / 96,
                  opacity: selectedItem.progress >= selectedItem.total ? 1 : 0.3,
                }}
              />
              <Text className="text-center">{selectedItem.desc}</Text>
              <View className="flex-row gap-2 items-center justify-between w-full">
                <Progress
                  value={Math.round((selectedItem.progress / selectedItem.total) * 100)}
                  className="h-3 w-full flex-1 bg-neutral-200 dark:bg-neutral-800"
                  indicatorClassName="bg-success"
                />
                <Text className="font-semibold">
                  {Math.min(selectedItem.progress, selectedItem.total)}/{selectedItem.total}
                </Text>
              </View>
