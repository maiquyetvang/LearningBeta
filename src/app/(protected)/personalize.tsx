...
const PersonalizeScreen: React.FC = () => {
      ...
      <View key={index} className="flex-row w-full gap-2 items-center justify-center">
        ...
        <View className="flex-1 px-5 py-3 rounded-[28px] shadow-popover shadow-sm bg-neutral-50 dark:bg-neutral-900">
          <AnimatedScreenWrapper>
            <Text className="text-xl font-medium text-foreground">{step.title}</Text>
          </AnimatedScreenWrapper>
        </View>
      </View>
    );
  ...
  const handlePersonalizeNext = async (value: string) => {
    ...
    setProgress((prev) => {
      ...
      let updated;
      if (exists) {
        updated = prev.map((step) => (step.index === currentIndex ? { ...step } : step));
      } else {
  ...
  const renderStep = (index: number, personalizeValues: Record<string, string>) => {
