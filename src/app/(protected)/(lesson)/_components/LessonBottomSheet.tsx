...
const LessonBottomSheet = ({
  ...
  if (isSkip) {
    ...
    icon = (
      <AlertTriangle size={30} className="text-yellow-500" stroke={'#eab308'} strokeWidth={2.5} />
    );
    titleText = 'Skipped!';
    titleClass = 'text-yellow-500';
  } else if (isFalse) {
    bgColor = isDarkColorScheme ? '#202f36' : '#ffdfe0';
    icon = <XIcon size={30} className="text-white" stroke={'#ff4b4b'} strokeWidth={5} />;
          ...
          <View className=" flex-col  gap-3  justify-between">
            <View className="flex-row items-center gap-3">
              ...
              <View className="flex-col">
                <Text className={`text-xl font-semibold ${titleClass}`}>{titleText}</Text>
                {!!description && !isSkip && (
                  <Text className={`whitespace-normal ${titleClass}`}>{description}</Text>
                )}
                {isSkip && (
            ...
            <Button
              className={`rounded-xl border-r-2 border-l-[0px] border-b-2 active:border-b-[1px] active:border-r-[1px] ${
                isSkip ? 'bg-yellow-400' : !isFalse ? 'bg-success dark:bg-success-700' : 'bg-error'
              }`}
              onPress={onNextStep}
            >
