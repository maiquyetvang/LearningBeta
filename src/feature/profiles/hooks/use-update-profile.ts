    ...
    mutationFn: async (profile: UpdateProfileOptions) => {
      try {
        ...
        if (existing) {
          ...
          await db.updateTable('profiles').set(profile).where('user_id', '=', user_id).execute();
          setProfile({ ...existing, ...profile, updated_at: new Date().toISOString() });
        } else {
          ...
          // Cập nhật lại profile trong store
