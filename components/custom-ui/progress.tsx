...
export default function StepProgressBar({
  ...
  const stepCount = length || progressStep.length;

  // Luôn tạo lại mảng animScales khi stepCount thay đổi
  const animScales = useMemo(
    () => Array.from({ length: stepCount }, () => new Animated.Value(1)),
    [stepCount]
  );

  // Tham chiếu lưu lại currentIndex trước đó
  ...
  return (
    ...
    >
      {Array.from({ length: length || progressStep.length }).map((_, i) => {
        let stepColor = "bg-neutral-100 dark:bg-neutral-800";

        // Bước hiện tại đã có kết quả
        if (i === currentIndex && progressStep[i]) {
